...
//    return 0;
    ...
    for(uint64_t i = 0;i < CameraNum;i ++)
    {
        ...
        if(DataBaseOperate::GetCameraUserName(i, HikDev.sDevName) == false)
        {
    ...
    if(MsgModuleCom.Open(USB0, 115200, 8, 1, 'N') < 0)
    {
        ServiceRun = false;
    }
    ...
    {
#if 0
        gettimeofday(&struTimeEnd, nullptr);
        ulTimeEnd = static_cast<uint64_t>(struTimeEnd.tv_sec) * 1000000 + static_cast<uint64_t>(struTimeEnd.tv_usec);
        if(ulTimeEnd - ulTimeStart >= 1000000)
        {
            gettimeofday(&struTimeStart, nullptr);
            ulTimeStart = static_cast<uint64_t>(struTimeStart.tv_sec) * 1000000 + static_cast<uint64_t>(struTimeStart.tv_usec);

            MsgModuleCom.writeBuffer(AtCmd, sizeof(AtCmd));
        }
        ...
        for(uint32_t i = 0;i < uiAlarmNum;i ++)
        {
            ...
            HikDevOperate::HikDevGetAlarmDevName(i, strAlarmDevName);
